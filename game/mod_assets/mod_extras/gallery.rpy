...
init python:
    ...
    class GalleryImage:
        ...
        def export(self):
            ...
            if self.sprite:
                renpy.show_screen("dialog", message="Спрайты не могут быть загружены в папку с галереей. Пожалуйста, попробуйте другое изображение.", ok_action=Hide("dialog"))
            else:
                try: 
                ...
                renpy.show_screen("dialog", message='Экспортировано "%s" в папку с галереей.' % self.name, ok_action=Hide("dialog"))
    ...
    s1a = GalleryImage("sayori 1", name="Сайори", sprite=True)
